...
document.querySelectorAll('.theme-btn').forEach(btn=>{
  ...
  btn.addEventListener('click',()=>{
    ...
    html.setAttribute('data-theme',theme);
    LS.set('pom_theme',theme); rainLight();
    document.querySelectorAll('.theme-btn').forEach(b=>b.classList.remove('active'));
    btn.classList.add('active');
    // redraw moon for new theme
    setTimeout(()=>drawMoon(document.getElementById('moonCanvas'), getMoonPhase(new Date())),60);
  });
});
...
document.getElementById('calMonthYear').textContent = `${MONTHS[TM].slice(0,3)} ${TY}`;
...
/* ──────────────────────────────
   STATS
────────────────────────────── */
const ageEl=document.querySelector('.ageOfPerson')||document.createElement('span');
// anniversary years shown in anniv card instead
...
────────────────────────────── */
let annivPhotos = LS.get('pom_anniv_photos',[]);  // [{dataURL}]
let annivIdx    = 0;
let annivTimer  = null;

const annivCard       = $('annivCard');
const annivSlideWrap  = $('annivSlideWrap');
const annivPlaceholder= $('annivPlaceholder');
const annivImg        = $('annivImg');
const annivDotsEl     = $('annivDots');
const annivPrev       = $('annivPrev');
const annivNext       = $('annivNext');
const annivYears      = $('annivYears');
const annivUpload     = $('annivUpload');

// Calculate anniversary years
const START = new Date('2022-04-20');
const DIFF  = (new Date() - START) / (1000*60*60*24*365.25);
annivYears.textContent = DIFF.toFixed(1);

function showSlide(idx){
  if(!annivPhotos.length){annivImg.classList.add('hidden');annivPlaceholder.style.display='';return}
  annivIdx=(idx+annivPhotos.length)%annivPhotos.length;
  annivImg.classList.remove('hidden');
  annivPlaceholder.style.display='none';
  annivImg.style.opacity='0';
  annivImg.src=annivPhotos[annivIdx].dataURL;
  annivImg.onload=()=>{annivImg.style.opacity='1'};
  // dots
  annivDotsEl.innerHTML='';
  annivPhotos.forEach((_,i)=>{
    const d=document.createElement('span'); d.className='anniv-dot'+(i===annivIdx?' on':'');
    d.addEventListener('click',()=>showSlide(i));
    annivDotsEl.appendChild(d);
  });
}
...
annivPrev.addEventListener('click',e=>{e.stopPropagation();showSlide(annivIdx-1);startAutoSlide()});
annivNext.addEventListener('click',e=>{e.stopPropagation();showSlide(annivIdx+1);startAutoSlide()});

annivCard.addEventListener('click',()=>{
  if(annivPhotos.length===0) annivUpload.click();
});
...
// Long-press anniv to upload more photos
let annivHold=null;
annivCard.addEventListener('pointerdown',()=>{annivHold=setTimeout(()=>annivUpload.click(),600)});
...
let waterData = LS.get('pom_water',{});  // { 'YYYY-M-D': {oz:0, log:[{time,oz}]}, goal:64 }
...
function updateWaterUI(){
  ...
  $('waterGoalInput').placeholder=waterGoal;

  renderWaterHistory();
}

function renderWaterHistory(){
  const hist=$('waterHistory'); hist.innerHTML='';
...
function drawMoon(canvas,phase){
  ...
  const mc=lt?'#e8e0f8':'#d4caf0';
  const sc=lt?'rgba(50,15,120,.78)':'rgba(5,0,15,.9)';
  ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.fillStyle=mc;ctx.fill();
...
(()=>{
  const moon=$('moonCanvas'), mn=$('moonName'), md=$('moonDetail');
...
const BOOKS={
  ...
  'dune':            {title:'Dune',                 author:'Frank Herbert',    cover:'https://covers.openlibrary.org/b/id/8370614-L.jpg',   s:'Paul Atreides navigates political intrigue and desert survival on Arrakis, the only source of the most valuable substance in the universe. A sweeping saga of power, ecology, and destiny.'},
  'hobbit':          {title:'The Hobbit',           author:'J.R.R. Tolkien',  cover:'https://covers.openlibrary.org/b/id/6979861-L.jpg',   s:'Bilbo Baggins is swept into an epic quest by Gandalf and dwarves to reclaim a lost kingdom from the dragon Smaug. A timeless adventure of courage and unexpected heroism.'},
  'silent-patient':  {title:'The Silent Patient',   author:'Alex Michaelides',cover:'https://covers.openlibrary.org/b/id/9272648-L.jpg',   s:'Alicia Berenson shoots her husband and never speaks again. Psychotherapist Theo Faber becomes obsessed with uncovering her motive — and discovers a truth he never expected.'},
  'midnight-library':{title:'The Midnight Library', author:'Matt Haig',       cover:'https://covers.openlibrary.org/b/id/10410782-L.jpg',  s:'Between life and death exists a library of infinite books, each a different life that could have been lived. Nora Seed must decide which life is truly worth living.'},
  'gone-girl':       {title:'Gone Girl',            author:'Gillian Flynn',   cover:'https://covers.openlibrary.org/b/id/8371112-L.jpg',   s:'Amy Dunne disappears on her fifth anniversary. Her husband becomes the prime suspect. Alternating unreliable narrators dissect marriage, media, and the psychology of deception.'},
  'martian':         {title:'The Martian',          author:'Andy Weir',       cover:'https://covers.openlibrary.org/b/id/8391800-L.jpg',   s:'Mark Watney is stranded alone on Mars. Armed with botany, ingenuity, and dark humor, he must survive and signal NASA — a triumph of human problem-solving.'}
};
...
function openBook(key){
  const bk=BOOKS[key]; const st=bkState[key]; if(!bk)return;
  curBook=key;
  ...
  if(bk.cover){const img=new Image();img.src=bk.cover;img.alt=bk.title;img.onerror=()=>{cov.innerHTML='📖'};cov.appendChild(img)}
  else cov.innerHTML='📖';
...
volEl.addEventListener('input',()=>{aud.volume=+volEl.value}); aud.volume=+volEl.value;
